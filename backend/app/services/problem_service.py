...
def _init_db():
    with _get_db() as conn:
        ...
        """)
        conn.commit()
        # Safe migration if table previously created without group_name
        try:
            conn.execute("ALTER TABLE problems ADD COLUMN group_name TEXT")
            conn.commit()
        except Exception:
            pass
...
class ProblemService:
    @staticmethod
    def get_all(category=None, status=None, author_id=None, author=None):
    ...
    def get_by_id(problem_id):
        with _get_db() as conn:
            ...
            if item.get("evidence_urls"):
                ...
                item["evidenceUrls"] = []
            item["impactCount"] = item.get("impact_count") or ""
            item["impactDescription"] = item.get("impact_description") or ""
            item["reporterType"] = item.get("reporter_type") or "Citizen"
            item["groupName"] = item.get("group_name") or ""
            return item
        ...
        with _get_db() as conn:
            conn.execute("""
                INSERT OR REPLACE INTO problems (
                    ...
                    latitude, longitude, thumbnail, evidence_urls, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                prob_id,
                title,
